  ...
  if ( auth( request, response ) ) {
    connection( ( db ) => {
      ...
      db.collection( 'accounts' ).aggregate(
        [ {
          $match: {
            resetCode: sanitize( request.body.resetToken )
          }
        }, {
          $project: {
            item: 1,
            dateDifference: {
              $subtract: [ new Date(), "$resetTime" ]
            }
          }
        } ],
        ( err, doc ) => {
          ...
          if ( err ) {
            ...
            if ( doc ) {
              if ( doc.length === 1 ) {
                ...
                response.json( {
                  valid: false,
                  message: 'No or Too Many Matching Documents.'
                } );
                db.close;
              }
            } else {
  ...
  if ( auth( request, response ) ) {
    ...
    connection( ( db ) => {
      ...
      db.collection( 'accounts' ).aggregate(
        [ {
          $match: {
            resetCode: sanitize( request.body.resetCode )
          }
        }, {
          $project: {
            item: 1,
      ...
      db.collection( 'accounts' ).findOne( {
        ...
        if ( err ) {
          ...
          if ( doc ) {
            ...
            response.json( {
              mailsent: false,
              message: 'Email was not in our system. Please set up a Free Account'
            } )
          }
        }
      } );
    ...
    connection( ( db ) => {
      ...
      db.collection( 'accounts' ).findOne( {
        ...
        if ( err ) {
          ...
          let id = sanitize( request.params.id );
          let str = `ObjectId("${id}")`;

          db.collection( 'accounts' ).update( {
            email: sanitize( doc.email )
          }, {
            $push: {
              codeFavorites: ObjectId( sanitize( request.params.id ) )
            }
          }, ( err, doc ) => {
            if ( err ) {
    ...
    connection( ( db ) => {
      ...
      if ( account._id ) {
        ...
        if ( checkFields( Account, account ) ) {
          ...
          response.json( {
            created: false,
            message: 'Missing fields or invalid'
          } );
        }
    ...
    if ( jem._id ) {
      ...
      db.collection( 'jems' ).updateOne( {
        ...
        if ( err ) {
          ...
          res.json( {
            status: 200,
            message: 'Jem Updated in the Database'
          } );
        }
      ...
      db.collection( 'jems' ).insertOne( jem, ( err, result ) => {
        ...
        if ( err ) {
          ...
          res.json( {
            status: 200,
            message: 'Jem Inserted into Database'
          } );
        }
...
app.get( '/session', ( req, res ) => {
  if ( auth( req, res ) ) {
    connection( ( db ) => {
      db.collection( 'accounts' ).findOne( {
        token: sanitize( req.cookies.token )
      }, ( err, doc ) => {
        if ( err ) {
          sendError( err, res );
        } else {
